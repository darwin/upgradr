...
{ \
	...
	HandleError(CUpgradrCOMError(_hr_, s).ErrorMessage()); \
	DebugBreak();\
}\
}
#else
...
typedef CComObject<CUpgradrScriptSite>		TScriptSite;

//////////////////////////////////////////////////////////////////////////
// Class that adds wstring support to runtimem_Error
class CUpgradrRuntimeError {
public:
	CUpgradrRuntimeError(const CString &message) : m_kMessage(message)
	{
	...
	CUpgradrRuntimeError(const CUpgradrRuntimeError &right) : m_kMessage(right.ErrorMessage())
	{
		LPTSTR str = m_kMessage.LockBuffer();
		TRACE_E(str);
		m_kMessage.UnlockBuffer();
	...
	CUpgradrRuntimeError& operator=(const CUpgradrRuntimeError &right)
	{
		m_kMessage = right.ErrorMessage();
	}

	virtual ~CUpgradrRuntimeError()
	{
	}
...
class CUpgradrWindowsError : public CUpgradrRuntimeError {
public:
	CUpgradrWindowsError(HRESULT error, const CString &message) : CUpgradrRuntimeError(message), m_Error(error) { }
	CUpgradrWindowsError(HRESULT error) : CUpgradrRuntimeError(GetMessageFromErrorCode(error)), m_Error(error) { }
	CUpgradrWindowsError(const CUpgradrWindowsError &right) : CUpgradrRuntimeError(right), m_Error(right.m_Error) { }
	virtual ~CUpgradrWindowsError() { }

	CUpgradrWindowsError& operator=(const CUpgradrWindowsError &right)
	{
		CUpgradrRuntimeError::operator=(right);
...
// Exception class for errors that occurred when calling Win32 API functions
class CUpgradrCOMError : public CUpgradrRuntimeError {
public:
	CUpgradrCOMError(HRESULT error, const CString &message) : CUpgradrRuntimeError(message+_T("\n")+GetMessageFromErrorCode(error)), m_Error(error) { }
	CUpgradrCOMError(HRESULT error) : CUpgradrRuntimeError(GetMessageFromErrorCode(error)), m_Error(error) { }
	CUpgradrCOMError(const CUpgradrCOMError &right) : CUpgradrRuntimeError(right), m_Error(right.m_Error) { }
	virtual ~CUpgradrCOMError() { }

	CUpgradrCOMError& operator=(const CUpgradrCOMError &right)
	{
		CUpgradrRuntimeError::operator=(right);
		m_Error = right.m_Error;
	}
...
// Exception class for errors that occurred when calling Win32 API functions
class CUpgradrSQLError : public CUpgradrRuntimeError {
public:
	CUpgradrSQLError(int error, const CString &message) : CUpgradrRuntimeError(message), m_Error(error) { }
	CUpgradrSQLError(int error) : CUpgradrRuntimeError(_T("?")), m_Error(error) { }
	CUpgradrSQLError(const CUpgradrSQLError &right) : CUpgradrRuntimeError(right), m_Error(right.m_Error) { }
	virtual ~CUpgradrSQLError() { }

	CUpgradrSQLError& operator=(const CUpgradrSQLError& right)
	{
		CUpgradrRuntimeError::operator=(right);
		m_Error = right.m_Error;
	}
...
BSTR GetUnicodeHTML(CComPtr<IHTMLDocument2> spDocument);