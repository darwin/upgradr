...
public:
	...
	HRESULT	InstallHook(HWND hWnd)
	{
		...
		if (m_hHook == NULL)
		{
			m_hHook = ::SetWindowsHookEx(WH_CBT, CBTProcStub, GetBaseModule().GetModuleInstance(), GetCurrentThreadId());
			// is the hook set?
			if (m_hHook == NULL)
			{
				ATLASSERT(false);
...
private:
	...
	static HWND GetDockingParent(HWND hWnd)
	{
		...
		if (!aClass)
		{
			WNDCLASSEX wc;
			wc.cbSize = sizeof(wc);
			aClass = (ATOM)::GetClassInfoEx(GetBaseModule().GetModuleInstance(),_T("CPackageWindowFrame::CPackageWindow"),&wc);
		}
