...
public:
    ...
    void UpdateLayout(BOOL bResizeBars = TRUE)
    {
	 ...
	 {
		 {
			 CWndPackage::iterator i = m_vPackage.m_frames.begin();
			 while (i!=m_vPackage.m_frames.end())
			 {
				 DFMHDR dockHdr;
				 ...
				 if (dockHdr.hWnd) 
				 {
					 dockHdr.hBar=::GetParent(dockHdr.hWnd);
					 CDocker(m_hWnd).Undock(&dockHdr);
					 i = m_vPackage.m_frames.begin();
					 continue;
				 }
				 ++i;
			 }
		 ...
		 {
			 CWndPackage::iterator i = m_hPackage.m_frames.begin();
			 while (i!=m_hPackage.m_frames.end())
			 {
				 DFMHDR dockHdr;
				 dockHdr.code=DC_UNDOCK;
				 dockHdr.hWnd=i->hwnd();
...
class TWinTraits = CDockingFrameTraits,
class TAutoHidePaneTraits = COutlookLikeAutoHidePaneTraits >
class ATL_NO_VTABLE CDockingFrameImpl2:
	public CDockingFrameImplBase< T, CFrameWindowImpl< T ,TBase, TWinTraits> ,TWinTraits, TAutoHidePaneTraits >
{
public:
	DECLARE_WND_CLASS(_T("CDockingFrameImpl2"))
};
