...
void OutputDebugStringFormat( LPCTSTR lpszFormat, ... )
{
	...
	vswprintf_s( lpszBuffer, lpszFormat, fmtList );
	va_end( fmtList );
...
void InitSymbolPath( PSTR lpszSymbolPath, PCSTR lpszIniPath )
{
   ...
   // ".;%_NT_SYMBOL_PATH%;%_NT_ALTERNATE_SYMBOL_PATH%;%SYSTEMROOT%;%SYSTEMROOT%\System32;"
	strcpy_s(lpszSymbolPath, BUFFERSIZE, ".");

	// environment variable _NT_SYMBOL_PATH
	if ( GetEnvironmentVariableA( "_NT_SYMBOL_PATH", lpszPath, BUFFERSIZE ) )
	{
	   strcat_s( lpszSymbolPath, BUFFERSIZE, ";" );
		strcat_s( lpszSymbolPath, BUFFERSIZE, lpszPath );
	}
	...
	if ( GetEnvironmentVariableA( "SYSTEMROOT", lpszPath, BUFFERSIZE ) )
	{
	   strcat_s( lpszSymbolPath, BUFFERSIZE, ";" );
		...
		// SYSTEMROOT\System32
		strcat_s( lpszSymbolPath, BUFFERSIZE, lpszPath );
		strcat_s( lpszSymbolPath, BUFFERSIZE, "\\System32" );
	}
   ...
   // Add user defined path
	if ( lpszIniPath != NULL )
		...
		{
		   strcat_s( lpszSymbolPath, BUFFERSIZE, ";" );
			strcat_s( lpszSymbolPath, BUFFERSIZE, lpszIniPath );
		}
}
...
BOOL GetModuleNameFromAddress( UINT address, LPTSTR lpszModule )
{
	...
	if ( SymGetModuleInfo( GetCurrentProcess(), (DWORD)address, &moduleInfo ) )
	{
	   ...
	   // Not found :(
		_tcscpy_s( lpszModule, BUFFERSIZE, _T("?") );
	...
	if ( SymGetSymFromAddr( GetCurrentProcess(), (ULONG)fnAddress, &dwDisp, pSym ) )
	{
      ...
      // I am just smarter than the symbol file :)
		if ( _tcscmp(lpszUnDSymbol, _T("_WinMain@16")) == 0 )
			_tcscpy_s(lpszUnDSymbol, _T("WinMain(HINSTANCE,HINSTANCE,LPCTSTR,int)"));
		else
		if ( _tcscmp(lpszUnDSymbol, _T("_main")) == 0 )
			_tcscpy_s(lpszUnDSymbol, _T("main(int,TCHAR * *)"));
		else
		if ( _tcscmp(lpszUnDSymbol, _T("_mainCRTStartup")) == 0 )
			_tcscpy_s(lpszUnDSymbol, _T("mainCRTStartup()"));
		else
		if ( _tcscmp(lpszUnDSymbol, _T("_wmain")) == 0 )
			_tcscpy_s(lpszUnDSymbol, _T("wmain(int,TCHAR * *,TCHAR * *)"));
		else
		if ( _tcscmp(lpszUnDSymbol, _T("_wmainCRTStartup")) == 0 )
			_tcscpy_s(lpszUnDSymbol, _T("wmainCRTStartup()"));
		...
		if ( _tcsstr( lpszUnDSymbol, _T("(void)") ) == NULL && _tcsstr( lpszUnDSymbol, _T("()") ) == NULL)
		{
			...
			for( ; ; index++ )
			{
				...
				*lpszParamSep = _T('\0');
				...
				_tcscat_s( lpszSymbol, BUFFERSIZE, lpszParsed );
				_stprintf_s( lpszSymbol + _tcslen(lpszSymbol), BUFFERSIZE-_tcslen(lpszSymbol), _T("=0x%08X,"), stackAddress + 2 + index );
...
BOOL GetSourceInfoFromAddress( UINT address, LPTSTR lpszSourceInfo )
{
	...
	if ( SymGetLineFromAddr( GetCurrentProcess(), address, &dwDisp, &lineInfo ) )
	{
	   // Got it. Let's use "sourcefile(linenumber)" format
		...
		_stprintf_s( lpszSourceInfo, BUFFERSIZE, _T("%s(%d)"), lpszFileName, lineInfo.LineNumber );
		ret = TRUE;
	}
	else
	{
      ...
      // Let's use the "modulename!address" format
		...
		if ( lpModuleInfo[0] == _T('?') || lpModuleInfo[0] == _T('\0'))
         ...
         // Let's use the "address" format
			_stprintf_s( lpszSourceInfo, BUFFERSIZE, _T("0x%08X"), address );
		else
			_stprintf_s( lpszSourceInfo, BUFFERSIZE, _T("%s!0x%08X"), lpModuleInfo, address );
