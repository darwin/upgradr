...
#define CTreeViewCtrl CTreeCtrl
...
public:
   ...
   DWORD m_dwExStyle;                               // Extended TreeListView styles
	...
	DWORD m_dwTreeStyle;                             // Style for treeview at creation

   CTreeListViewImpl() : 
      m_cxHeader(0),
      m_nOffset(0),
      m_dwExStyle(0),
      m_dwHeaderStyle(WS_CHILD | WS_VISIBLE | HDS_BUTTONS | HDS_FILTERBAR | HDS_HORZ),
		m_dwTreeStyle(WS_TABSTOP | WS_CHILD | WS_VISIBLE |	WS_CLIPSIBLINGS |	WS_CLIPCHILDREN |	TVS_HASLINES | TVS_HASBUTTONS | TVS_DISABLEDRAGDROP)
   { 
   }
   ...
   CTreeViewCtrl* GetTreeControl() 
   {
      ...
      return (CTreeViewCtrl*)&m_ctrlTree;
   }
   CHeaderCtrl* GetHeaderControl() 
   {
      ATLASSERT(::IsWindow(m_ctrlHeader));
      return (CHeaderCtrl*)&m_ctrlHeader;
   }
   ...
   void _Init()
   {
      ...
      m_ctrlTree.Create(this, 1, m_hWnd, &rcDefault, NULL, m_dwTreeStyle, 0, nID);
